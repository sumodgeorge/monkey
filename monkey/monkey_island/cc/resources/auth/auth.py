...
class Authenticate(flask_restful.Resource):
    ...
    def post(self):
        ...
        if self._authenticate(username, secret):
            username = str(user_store.UserStore.username_table[username].id).encode()
            access_token = flask_jwt_extended.create_access_token(identity=username)  # Expects bytes
